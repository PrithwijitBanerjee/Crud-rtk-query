...
import { useGetSingleStudentQuery, useUpdateStudentMutation } from '../Services/api';
const initialState = {
  name: '',
  age: '',
  gender: '',
...
const EditStudent = () => {
  ...
  const submitInfo = e => {
    ...
    if (Object.keys(errorList)?.length < 1) {
      //Async Operation...
      editStudent({id,studentData:student});
    ...
    <>
      <div className="card" style={{ width: '30rem', margin: '50px auto' }}>
        <div className="card-body">
                ...
                <div className="form-group">
                  ...
                  <div>
                    <div className="form-check form-check-inline">
                      <input className="form-check-input" type="radio" name="gender" id="inlineRadio1" defaultValue="option1" value='Male' onChange={postUserData} checked={(student?.gender==='Male')?true:false}/>
                      <label className="form-check-label" htmlFor="inlineRadio1">Male</label>
                    </div>
                    <div className="form-check form-check-inline">
                      <input className="form-check-input" type="radio" name="gender" id="inlineRadio2" defaultValue="option2" value='Female' onChange={postUserData} checked={(student?.gender==='Female')?true:false}/>
          ...
          <button className="btn btn-outline-success" onClick={submitInfo}>Edit</button>
        </div>
      </div>
